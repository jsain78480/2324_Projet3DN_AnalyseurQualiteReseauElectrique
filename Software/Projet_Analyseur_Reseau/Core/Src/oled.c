...
HAL_StatusTypeDef SCREEN_SSD1306_Init(__SCREEN_SSD1306_HandleTypeDef *SCREEN_SSD1306_Handle, I2C_HandleTypeDef *hi2c){
	...
	if(HAL_OK == HAL_I2C_IsDeviceReady(SCREEN_SSD1306_Handle->hi2c, SCREEN_SSD1306_Handle->i2c_addr, 3, 100)){
		tx_Buffer_Size = snprintf((char *)uart1_Tx_Buffer, UART_TX_BUFFER_SIZE, "Screen connected\r\n");
		HAL_UART_Transmit(&huart1, uart1_Tx_Buffer, tx_Buffer_Size, 10);
	}
	else{
		tx_Buffer_Size = snprintf((char *)uart1_Tx_Buffer, UART_TX_BUFFER_SIZE, "Screen not connected\r\n");
		HAL_UART_Transmit(&huart1, uart1_Tx_Buffer, tx_Buffer_Size, 10);
	}
